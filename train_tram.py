...
def train(config):
  ...
  model = TRAM(state_dict,args.layer,args.config)
  ...
  for param in model.clip_backbone.visual.parameters():
  ...
  iter_per_epoch = int(args.iter_check)
  ...
  for iter_num in range(0, args.iter + 1):
    ...
    classifier_label_out,label,cls_loss,txt_loss,view_loss,fg_sm,multi_label,attn_loss = model.triplet(input_image,input_image_mask,input_label,input_caption,input_indicators,iter_num)
    ...
    total_loss = args.alpha * cls_loss  + attn_loss
    ...
    for name, param in model.named_parameters():
        if torch.isnan(param).any() or torch.isinf(param).any():
            print("NaN/Inf in weights BEFORE step:", name)
            raise SystemExit
    ...
    for name, param in model.named_parameters():
        if torch.isnan(param).any() or torch.isinf(param).any():
            print("NaN/Inf in weights AFTER step:", name)
            raise SystemExit
    optimizer.zero_grad()


    cls_hist_loss.append(cls_loss.detach().cpu().numpy())
    attn_dist_hist_loss.append(attn_loss.detach().cpu().numpy())
    ...
    if (iter_num == 0 or (iter_num + 1) % (iter_per_epoch) == 0):
        ...
        print('attn dist loss')
        attn_dist_hist_loss = np.array(attn_dist_hist_loss)
        print(attn_dist_hist_loss.sum()/len(attn_dist_hist_loss))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--iter_check', type=int, default=100, help='number of iterations')
  ...
  parser.add_argument('--shrink_pixel', type=int, default=5,help='shrink_pixel of mask')
  parser.add_argument('--pretrain_path', type=str, default=None)
  parser.add_argument('--batch_size', type=int, default=2)
  parser.add_argument('--attn_iter', type=int, default=0)
